  ...
  module Helpers
    module LightBoxHelper
      ...
      module InstanceMethods
        def lightbox_image_tag(source, destination, album=nil, image_options = {}, html_options = {})
          album = "[#{album}]" if album
          html_options.merge!(:rel => "lightbox#{album}") unless html_options[:rel]
          html_options.merge!(:title => "") unless html_options[:title]
          link_to(image_tag(source, image_options), destination, html_options)
        end
        
        def lightbox_link_to(name, options = {}, album=nil, html_options = {}, *parameters_for_method_reference)
          album = "[#{album}]" if album
          html_options.merge!(:rel => "lightbox#{album}") unless html_options[:rel]
          html_options.merge!(:title => "") unless html_options[:title]
